...
router.route('/users/signup').post(validateBody(schemas.authSchema), cors(), userController.signup);

//signin
router.route('/users/signin').post(validateBody(schemas.signinSchema), passportSignin, cors(), userController.signin);

//secret
router.route('/users/secret').get(passportjwt, cors(), userController.secret);

//signin with google
router.route('/oauth/google').post(passportGoogle, userController.googleSignin);

//signin with facebook
router.route('/oauth/facebook').post(passportFacebook, userController.facebookSignin);
...
router
	.route('/users/:Id/campaign')
	.get(cors(), userController.getUserCampaigns)
	.post(cors(), upload.single('image'), userController.createUserCampaign);
//update campaign
router.route('/users/:Id/campaign').patch(cors(), upload.single('image'), userController.updateUserCampaign);
...
router
	...
	.put(cors(), upload.single('image'), userController.updateUserCampaign);

//show all campaigns
router.route('/campaigns').get(cors(), campaignController.getAllCampaigns);
